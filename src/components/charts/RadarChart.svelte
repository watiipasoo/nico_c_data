<script lang="ts">
    ...
    function updateChart() {
      const selectedDatasets = selectedData.map((dataKey) => {
        return {
          ...
          backgroundColor: 'rgba(0, 123, 255, 0.2)', // Background color for the radar area
          borderWidth: 2,
        };
      });
    ...
    onMount(() => {
      ...
      chart = new Chart(ctx, {
        type: 'radar', // Use 'radar' for a radar chart
        data: {
        ...
        options: {
          ...
          elements: {
            line: {
              tension: 0.0, // Make the radar lines straight
            },
          },
        },
      });
  ...
  <div class="flex">
    <div class="flex-[100%]">
      ...
      <canvas id="radarChart" /> <!-- Use 'radarChart' for the canvas ID -->
    </div>
  </div>
